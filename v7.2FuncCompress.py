...
def report(name, rec_rgb, orig_rgb, band_mask, interior_mask):
    ...
    print(f"{name}:")
    print(f"  full      MSE={full_mse:.4f}, PSNR={full_psnr:.3f}, SSIM={full_ssim:.4f}, LPIPS={full_lpips:.4f}")
...
report("A exact+mean", A_rgb, orig_rgb, band_mask, interior_mask)
report("B spline+mean", B_rgb, orig_rgb, band_mask, interior_mask)
...
# Finite-difference decomposition
dseg = dC
dbnd = dF - dC
dapp = dD - dC
dpred = dseg + dbnd - dapp
interaction = dE - dpred

print("\nMSE decomposition:")
print(f"  dseg        = dC             = {dseg:.4f}")
print(f"  dbnd        = dF - dC       = {dbnd:+.4f}   (dF={dF:.4f})")
print(f"  dapp        = dD - dC       = {dapp:+.4f}   (dD={dD:.4f})")
print(f"  dpred       = dseg + dbnd - dapp  = {dpred:.4f}")
print(f"  dE (actual)                 = {dE:.4f}")
print(f"  interaction = dE - dpred    = {interaction:+.4f}  ({100*interaction/max(dE, 1e-9):.1f}% of dE)")
